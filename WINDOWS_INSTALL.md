...
Смотри на блок `Готовность к прошивке`: если там `ГОТОВО`, значит устройство и прошивка уже проверены.

В приложении также есть `Центр релизов Gradus`, где можно быстро увидеть актуальный релиз и список доступных бинарников.
...
Для установочной версии используй GitHub Release с файлом `GradusFlasher-Setup-v0.1.3.exe`.

## 11. Portable-режим

Если хочешь переносимую версию без системных настроек:

- создай рядом с `GradusFlasher.exe` файл `portable.ini`;
- после этого приложение будет хранить настройки рядом с собой в локальном `.ini`.

## 12. Экспорт логов

- используй кнопку `Экспорт логов` в верхней панели;
- или кнопку `Сохранить лог в файл` в блоке истории операций.

## 13. Самый короткий путь
