...
> repo: m5-flasher
> doc: INSTALL
> purpose: Установка, запуск и первая прошивка Gradus Flasher
```
...
В блоке `Готовность к прошивке` приложение крупно показывает, чего еще не хватает: проверки устройства или анализа `.bin`.

В блоке `Центр релизов Gradus` можно посмотреть актуальный релиз, дату публикации и список доступных release-артефактов.
...
- если рядом с приложением или в корне проекта лежит `portable.ini`, настройки сохраняются локально в portable-режиме.

## 10. Экспорт логов

- кнопка `Экспорт логов` находится в верхней панели;
- кнопка `Сохранить лог в файл` есть в блоке истории операций;
- лог можно сохранить в `.log` или `.txt`.

## 11. Рекомендуемый сценарий установки
