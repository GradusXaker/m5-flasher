...
Если нужен отдельный Windows-only гайд с командами под `PowerShell`, смотри `WINDOWS_INSTALL.md`.
...
7. Нажми `Проверить устройство`, чтобы приложение попробовало определить чип, flash и подсказать подходящий профиль.
8. Убедись, что `offset` равен `0x0`, если у тебя обычный single-image `.bin`.
9. При необходимости переведи плату в boot/download mode.
10. Нажми `ПРОШИТЬ УСТРОЙСТВО`.
11. Следи за логом и прогресс-баром.

Если хочешь более простой сценарий, используй кнопку `Мастер прошивки` в верхней части главного окна.
